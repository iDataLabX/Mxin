...
## 评测标准
### 1. 评测方式
墨心评测（MxinBench）提出一种“领域知识蒸馏—评委模型对齐—自动化推理评测”三阶段级联范式，旨在将人类专家的鉴赏认知系统性地蒸馏至可规模化部署的评委模型中，从而对待测系统的鉴赏输出执行自动化、可复现、逐维度的精细化评分。该范式的核心设计理念在于：以领域专家的结构化审美判断为锚定信号，经由有监督对齐将其内化为评委模型的评分策略，最终实现专家级判断力迁移。
...
阶段二：多维结构化专家标注（Multi-Dimensional Structured Expert Elicitation）。 本阶段采用结构化专家知识引出协议，由美术史学者、国画创作者等构成的跨学科专家组依据7大任务类型对每幅画作实施多维度、多粒度的结构化标注。每条标注条目包含参考答案、评分锚点与专家阐释三重信息层，共同构成黄金标注语料库。

阶段三：评委模型的有监督对齐与评分校准（Supervised Alignment & Score Calibration of Judge Model）。 本阶段采用 MLLM-as-Judge 技术路线，以黄金标注语料为监督信号，通过两步训练策略构建专用评委模型：首先，由指令微调使基座多模态大语言模型习得面向中国画鉴赏场景的多维度评分范式与审美推理链路能力；继而，引入评分校准阶段，通过 Sigmoid 映射函数对模型原始输出进行非线性校准，使其评分分布逼近专家人工评分的统计特征。经校准后的评委模型与专家评分之间的一致性以组内相关系数（Intraclass Correlation Coefficient, ICC）为度量指标，目标达到 ICC ≥ 0.75 的良好一致性水平，从而在保障评测专业性的同时实现规模化、标准化、可复现的自动评估。


### 2. 评测维度

MxinBench 构建了一套“识画—懂画—传画—通心”四层递进式鉴赏能力评测框架，辅以贯穿全局的忠实性约束层，共形成 5 大能力层级、12 项原子化评测维度。设计逻辑遵循中国画鉴赏由表及里的认知递进规律：以“识画”为视觉基座，锚定模型对画面要素与文化符号的感知解码能力；以“懂画”为认知纵深，从情感、意境、知识三维考察模型对画作深层语义的理解与关联能力；以“传画”为表达双翼，衡量模型将内隐鉴赏认知转化为兼具文学品质与视觉锚定性的外显文本之能力；以“通心”为交互旨归，评估模型在多轮对话中的情感深化引导、受众适配、人格具身与情境感知能力。最后，”全局约束”层作为贯穿所有任务的底线性保障，确保阐释忠实性与评价校准性。五层鉴赏能力体系共同刻画出中国画鉴赏的完整能力剖面。
...
MxinBench 构建了一套端到端的艺术情感交互评测集，以真实博物馆对话场景为锚点，通过对艺术作品的深度讲解与个性化情感引导，系统评估多模态大模型在中国画鉴赏中的视觉理解、文化认知、审美判断、文学表达与情感共鸣能力。评测集选取横跨唐、宋、明、清四朝、涵盖人物仕女、山水、花卉竹石、青绿山水等主要画科的 5 幅经典传世名作为种子画作，围绕每幅画作设计 7 大任务类型、共 9 个评测单元，覆盖全部 12 项评测维度（D1–D12）。
...
![](images/qianli_jiangshan_wangximeng.jpg)
